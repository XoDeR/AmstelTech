...
namespace RioCore
{
	...
	public:
		...
		uint16_t m_maxHandles = 0;

		HandleAlloc(uint16_t maxHandles)
	...
	inline HandleAlloc* createHandleAlloc(AllocatorI* allocator, uint16_t maxHandles)
	{
		uint8_t* ptr = (uint8_t*)RIO_ALLOC(allocator, sizeof(HandleAlloc) + 2 * maxHandles*sizeof(uint16_t));
		return ::new (ptr) HandleAlloc(maxHandles);
	}

	inline void destroyHandleAlloc(AllocatorI* _allocator, HandleAlloc* _handleAlloc)
	{
		_handleAlloc->~HandleAlloc();
		RIO_FREE(_allocator, _handleAlloc);
	}
	...
	public:
		...
		{
			RIO_CHECK(isValid(_handle), "Invalid handle %d!", _handle);
	...
	public:
		...
		bool next(Iterator& _it) const
		{
			...
			for (;_it.pos < MaxCapacityT && invalid == m_handle[_it.pos]; ++_it.pos);
