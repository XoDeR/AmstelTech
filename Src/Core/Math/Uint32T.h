...
#if RIO_PLATFORM_WINDOWS || RIO_PLATFORM_XBOXONE || RIO_PLATFORM_WINRT
#include <math.h> 
// math.h is included 
...
#define RIO_HALF_FLOAT_ZERO UINT16_C(0)
#define RIO_HALF_FLOAT_HALF UINT16_C(0x3800)
#define RIO_HALF_FLOAT_ONE UINT16_C(0x3c00)
#define RIO_HALF_FLOAT_TWO UINT16_C(0x4000)
...
namespace RioCore
{
...
#elif RIO_COMPILER_MSVC && RIO_PLATFORM_WINDOWS
	...
	inline uint64_t uint64_cntbits_ref(uint64_t _val)
	{
		...
		const uint32_t total = RioCore::uint32_cntbits(lo)
			+ RioCore::uint32_cntbits(hi);
...
#elif RIO_COMPILER_MSVC && RIO_ARCH_64BIT
...
#elif RIO_COMPILER_MSVC && RIO_PLATFORM_WINDOWS && RIO_ARCH_64BIT
		unsigned long index;
	...
	namespace Uint32
	{
		...
		inline uint32_t getGreatestCommonDivisor(uint32_t a, uint32_t b)
		{
			do
			{
				uint32_t tmp = a % b;
		...
		inline uint32_t getLeastCommonMultiple(uint32_t a, uint32_t b)
		{
			return a * (b / getGreatestCommonDivisor(a, b));
		}

		// Align to arbitrary stride and 16-bytes
		inline uint32_t strideAlign16(uint32_t offset, uint32_t stride)
		{
			const uint32_t align = getLeastCommonMultiple(16, stride);
			const uint32_t mod = uint32_mod(offset, align);
			const uint32_t mask = uint32_cmpeq(mod, 0);
			const uint32_t tmp0 = uint32_selb(mask, 0, align);
			const uint32_t tmp1 = uint32_add(offset, tmp0);
		...
		inline uint32_t strideAlign256(uint32_t offset, uint32_t stride)
		{
			const uint32_t align = getLeastCommonMultiple(256, stride);
			const uint32_t mod = uint32_mod(offset, align);
			const uint32_t mask = uint32_cmpeq(mod, 0);
			const uint32_t tmp0 = uint32_selb(mask, 0, align);
			const uint32_t tmp1 = uint32_add(offset, tmp0);
