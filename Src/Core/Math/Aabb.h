...
namespace AabbFn
{
	...
	// Returns the center of the box <b>
	Vector3 getCenter(const Aabb& b);

	// Returns the radius of the box <b>
	float getRadius(const Aabb& b);

	// Returns the volume of the box <b>
	...
	Aabb getTransformedByMatrix(const Aabb& b, const Matrix4x4& m);

	// Returns the eight vertices of the box <b>
	void getBoxVertexList(const Aabb& b, Vector3 v[8]);

	// Returns the sphere enclosing the box <b>
	Sphere getEnclosingSphere(const Aabb& b);
...
namespace AabbFn
{
	...
	inline Aabb getTransformedByMatrix(const Aabb& b, const Matrix4x4& m)
	{
		Vector3 vertices[8];

		getBoxVertexList(b, vertices);
	...
	inline void getBoxVertexList(const Aabb& b, Vector3 v[8])
	{
		// 7 ---- 6
		// |      |		<-- top face
		// |      |  
	...
	inline Sphere getEnclosingSphere(const Aabb& b)
	{
		Sphere s;
