...
namespace RioCore
{
	...
	inline float vec3Dot(const float* __restrict a, const float* __restrict b)
	{
	...
	inline void vec3TangentFrame(const float* __restrict _n, float* __restrict _t, float* __restrict _b)
	{
		...
		if (RioCore::fabsolute(nx) > RioCore::fabsolute(nz))
		{
			float invLen = 1.0f / RioCore::fsqrt(nx*nx + nz*nz);
			_t[0] = -nz * invLen;
			_t[1] = 0.0f;
			_t[2] = nx * invLen;
		}
		...
		{
			float invLen = 1.0f / RioCore::fsqrt(ny*ny + nz*nz);
			_t[0] = 0.0f;
			_t[1] = nz * invLen;
			_t[2] = -ny * invLen;
		}

		RioCore::vec3Cross(_b, _n, _t);
	}
	...
	{
		const float height = 1.0f / tanf(getRadiansFromDegrees(_fovy)*0.5f);
	...
	inline void mtxProjInfXYWH(float* _result, float _x, float _y, float _width, float _height, float _near, bool _oglNdc = false)
	{
		float aa;
		float bb;
		if (RIO_ENABLED(NearFar::Reverse == NearFarT))
		{
	...
	inline void mtxMul(float* __restrict result, const float* __restrict _a, const float* __restrict _b)
	{
		vec4MulMtx(&result[0], &_a[0], _b);
		vec4MulMtx(&result[4], &_a[4], _b);
		vec4MulMtx(&result[8], &_a[8], _b);
		vec4MulMtx(&result[12], &_a[12], _b);
	}

	inline void mtxTranspose(float* __restrict result, const float* __restrict _a)
	{
		result[0] = _a[0];
		result[4] = _a[1];
		result[8] = _a[2];
	...
	inline void mtx3Inverse(float* __restrict result, const float* __restrict _a)
	{
		float xx = _a[0];
		float xy = _a[1];
		float xz = _a[2];
	...
	inline void mtxInverse(float* __restrict result, const float* __restrict _a)
	{
		float xx = _a[0];
		float xy = _a[1];
		float xz = _a[2];
	...
	inline void mtxProjFlipHandedness(float* __restrict dst, const float* __restrict _src)
	{
		dst[0] = -_src[0];
		dst[1] = -_src[1];
		dst[2] = -_src[2];
	...
	inline void calcPlane(float result[4], float _va[3], float _vb[3], float _vc[3])
	{
		float normal[3];
	...
	inline void hsvToRgb(float rgb[3], const float hsv[3])
	{
		const float hh = hsv[0];
		const float ss = hsv[1];
		const float vv = hsv[2];
		...
		rgb[0] = vv * flerp(1.0f, fsaturate(px - 1.0f), ss);
		rgb[1] = vv * flerp(1.0f, fsaturate(py - 1.0f), ss);
		rgb[2] = vv * flerp(1.0f, fsaturate(pz - 1.0f), ss);
	}
